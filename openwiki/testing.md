...
public function test_invoice_can_be_duplicated()
{
    ...
    $this->assertGreater(
        Document::where('company_id', company_id())->count(),
        1
    );
}
```
...
**Primary Seeder**: `database/seeds/Database/Seeders`
...
public function test_currency_conversion_fetches_rates()
{
    Http::fake([
    ...
    $rate = CurrencyService::convert('USD', 'EUR', 100);
    
    $this->assertEquals(92, $rate);
}
```
...
```php
use Illuminate\Testing\Benchmark\Benchmark;

public function test_document_calculation_performance()
{
    $invoice = Document::factory()
        ->has(DocumentItem::factory()->count(100))
        ->create();
