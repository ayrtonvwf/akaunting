...
- Edges are 88.2% `EXTRACTED` (source-derived, confidence 1.0) and 11.8% `INFERRED` (verify in source). No edge currently carries `AMBIGUOUS`.
...
`graphify-out/.graphify_analysis.json` is a deterministic clustering/analysis sidecar, no LLM involved: the `cluster-only` step regenerates it on every rebuild alongside `graph.json`. Don't hand-edit it or revert it to a stale version — that would desync it from the current `graph.json`/community structure.
